...
export default function BankDashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            // 1. Fetch Statistics
            const statsResponse = await fetch(`${API_URL}/api/bank/statistics`, {
            ...
            if (statsResponse.ok) {
                ...
                setStats({
                    newApplications: data.pending_applications,
                    approved: data.approved_applications,
                    rejected: 0, // Backend doesn't return this yet
                    pending: data.pending_applications,
                    averageScore: data.average_score,
                    totalLoans: 0, // Need finance endpoint
                    overduePayments: 0,
                    overduePercentage: 0
                });
            }
            ...
            const appsResponse = await fetch(`${API_URL}/api/bank/applications?status=pending`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            ...
            if (appsResponse.ok) {
                ...
                const recentApps = appsData.slice(0, 5).map((app: any) => ({
                    id: app.id,
                    farmerName: app.farmer_name,
                    amount: app.loan_amount,
                    riskScore: app.ai_score || 0,
                    riskCategory: app.risk_category || "Unknown",
                    status: app.status,
                    date: new Date(app.date_submitted).toLocaleDateString()
                }));
